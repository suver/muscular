...
def basepath_constructor(loader, node):
    ...
    list_path.insert(0, Configurator.basedir)
...
def include_constructor(loader, node):
    ...
    if os.path.isdir(os.path.join(Configurator.basedir, file)):
        values = []
        for entry in glob.glob('/'.join([os.path.join(Configurator.basedir, file), '*.yaml'])):
            values.append(yaml.load(open(entry), Loader=yaml.FullLoader))
        return values
    else:
        extension = ''.join(pathlib.Path(os.path.join(Configurator.basedir, file)).suffixes)
        if extension == '.yaml':
            return yaml.load(open(os.path.join(Configurator.basedir, file)), Loader=yaml.FullLoader)
        else:
            return open(os.path.join(Configurator.basedir, file)).read()
...
def secret_constructor(loader, node):
    ...
    values = yaml.load(open(os.path.join(Configurator.basedir, './config/secret.yaml')), Loader=yaml.FullLoader)
...
class Configurator:
    ...
    def __init__(self, obj: typing.Optional[dict] = None, file: str = "configuration.yaml", basedir: str = None):
        ...
        try:
            ...
            if obj:
                ...
                self._object = yaml.load(open(os.path.join(Configurator.basedir, file)), Loader=yaml.Loader)
                for key in self._object.get('params', {}):
                    self._params.update({key: self._object['params'][key]})
        except ValueError as e:
            raise ConfiguratorConfigFileNotFound()
    ...
    def __repr__(self):
        if isinstance(self._object, str) or isinstance(self._object, int) or isinstance(self._object, bool):
            return self.get()
        else:
            return self._object
