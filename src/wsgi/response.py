...
class Response:
    """
    Объект ответа на зпрос
    """

    def __init__(self, status, reason=None, headers: list = [], body=None, file: str = None):
        """
        Конструктор ответа

        :param status: HTTP Код статуса ответа 200=ОК
        :param reason: Расшифровка статуса ответа
        :param headers: Заголовки ответа
        :param body: Тело ответа
        :param file: Файлы ответа
        """
        headers = sorted(headers, key=lambda d: d[0])

        self.status = status
        self.reason = code_status[str(status)]['message'] if str(status) in code_status and not reason else reason
        self.headers = headers
        self.body = body
        self.file = file

    @staticmethod
    def abort(http_status, reason=None):
        """
        Прерываем запрос с кодом и тектом
        :param http_status: HTTP Код статуса ответа 404=Not Found
        :param reason: Расшифровка статуса ответа
        :return:
        """
        raise MuscularError(http_status, reason)

    @staticmethod
    def body(status, headers: list = [], body=None):
        """
        Тело ответа
        :param status: HTTP Код статуса ответа 200=ОК
        :param list headers: Заголовки ответа
        :param body: Тело ответа
        :return: Response
        """
        ...
        headers = [(str(header[0]), str(header[1])) for header in headers if condition(header)]
    ...
    @staticmethod
    def error(status, reason=None, body=None, headers: list = []):
        """
        Формируем ошибку ответа
        ...
        return Response(status, headers=headers, body=body, reason=reason)

    @staticmethod
    def file(status, reason=None, headers: list = [], file: str = None):
        """
        ...
        return Response(status, headers=headers, file=file)
    ...
    @staticmethod
    def redirect(redirection, status=None, reason=None):
        """
        формируем редирект на другой адрес
        :param redirection: адрес для редиректа
        ...
        if isinstance(redirection, tuple):
            ...
            status = status if status else 307
            url = redirection
        return Response(status, reason=reason, headers=[('Location', url)])
    ...
    def not_found(reason=None):
        ...
        :return:
        """
        return Response(404, reason=reason)
