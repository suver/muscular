...
class Itinerary:
    ...
    def modify_response(self, response) -> Response:
        """
        Модифицирует объект ответ
    ...
    def match_with_params(self, url):
        """
        Возврщает подходящий маршрут с параметрами
    ...
    def add(self, route, key=None, handler=None, method=None, content_type=None,
        ...
        for c in self.nodes_map:
            if route == c['route'] and content_type == c['content_type'] and key == c['key'] and method == c['method']:
                self.logger.error(self.locale('Роутер должен иметь уникальную последовательность '
                                              '`key` [%s], `content_type` [%s], `method` [%s], `route` [%s]',
                                              key, content_type, method, route))
                raise Exception(self.locale('Роутер должен иметь уникальную последовательность '
                                            '`key` [%s], `content_type` [%s], `method` [%s], `route` [%s]',
                                            key, content_type, method, route))

        if handler is None:
            self.logger.error(self.locale('Параметр `handler` обязателен'))
            raise Exception(self.locale('Параметр `handler` обязателен'))

        node = self.node
        if key is None or not key:
            key = '.'.join(
                tuple(chunks[1:] if chunks[0] == '' else chunks)
            )
        _key, key = key, None
        ...
        self.logger.info(self.locale('Добавлен роутер %s (Key: %s, Content Type: %s, Method: %s, Redirect: %s)',
                                     route, node.key, content_type or '*/*', method or '*', redirect or 'None'))
        handler.node = node
        handler.full_route = full_route
        if not hasattr(handler, 'method') or not handler.method:
    ...
    def controller(self, route, *args, model: Schema = None, **kwargs):
        """
    ...
    def action(self, *args, route=None, key=None, module=None, method=None, content_type=None,
               redirect: str = None, model: Schema = None, **kwargs):
        """
...
class Node(ABC):
    ...
    def instance(self, chunk_route, key=None, full_route=None, dictionary_key=None, rule=None):
        ...
        if self.get_children_node(chunk_route):
            ...
            return Node(chunk_route, key=key, full_route=full_route, dictionary_key=dictionary_key, rule=rule, parent=self)
