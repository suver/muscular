...
class CommandPostgresDatabaseLinkDriver(CommandDatabaseLinkDriver):
    """
...
class SchemaPostgresDatabaseLinkDriver(DatabaseLinkDriver):
    """
    Link объект для управления схемами БД
    """
    ...
    def _to_sql_type(self, column) -> str:
        ...
        if hasattr(column.field_type, 'length'):
            field_param = '({value})'.format(
                value=column.field_type.length) if column.field_type.length is not None else ''
        if column.field_type.data_type == 'key':
            ...
            field = 'TIMESTAMP'
            field_param = '({value})'.format(
                value=column.field_type.timezone) if column.field_type.timezone is not None else ''
...
class Postgres(DataDriver):
    ...
    def __init__(self, name=None):
        ...
        self._mappers = [
            ...
            CommandPostgresDatabaseLinkDriver(self),
            SchemaDatabaseLinkDriver(self),
            CRUDDatabaseLinkDriver(self),
            SchemaPostgresDatabaseLinkDriver(self),
